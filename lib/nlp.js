...
export const EXPLICIT_INDUSTRY_PATTERNS = [
  ...
  { cat:"AI",                    rx:/\b(artificial\s+intelligence|machine\s+learning)\s+(jobs?|roles?|positions?|work|industry|sector|field|opportunities?)\b|\b(ai|artificial\s+intelligence)\s+(engineer|developer|researcher|scientist|specialist|jobs?|roles?)\b/i },
...
export function parseJobQuery(rawQuery) {
  ...
  // Experience
  const expPatterns = [
    { rx:/\b(entry[\s-]?level?|fresher|new\s+grad(uate)?|fresh\s+grad(uate)?|no\s+experience|no\s+exp|0[\s-]+years?|recently\s+grad(uated)?|just\s+graduated|new\s+graduate|just\s+started|fresh\s+out\s+of|recent\s+grad(uate)?|no\s+experience\s+required|0[\s-]?[12][\s-]+(years?|yrs?))\b/i, val:"Fresher" },
    { rx:/\b(junior|associate\b)\b/i,                                                                  val:"Junior" },
    { rx:/\b(internship|intern\b)/i,                                                                   val:"Fresher" },
    // Seniority words — placed EARLY so explicit year counts override them (since later wins)
    { rx:/\b(senior|sr\b|lead|principal|staff|experienced\b)/i,                                        val:"Senior" },
    { rx:/\b(?:with\s+)?1[\s-]+years?(?:\s+(of\s+)?(exp|experience))?\b/i,                           val:"Junior" },
    { rx:/\b(?:with\s+)?2[\s-]+years?(?:\s+(of\s+)?(?:\w+\s+)?(exp|experience))?\b/i,               val:"Junior" },
    { rx:/\b(?:with\s+)?3[\s-]+years?(?:\s+(of\s+)?(?:\w+\s+)?(exp|experience))?\b/i,               val:"Mid-level" },
    { rx:/\b(mid[\s-]?level?|intermediate|3[\s-]+to[\s-]+5)\b/i,                                     val:"Mid-level" },
    { rx:/\b[34][\s-]+years?\s+(of\s+)?(exp|experience)/i,                                            val:"Mid-level" },
    { rx:/\b3plus\s*(years?|yrs?)/i,                                                                   val:"Mid-level" },
    { rx:/\b5[\s-]+(years?|yrs?)(\s+(of\s+)?(exp|experience))?\b/i,                                   val:"Mid-level" },
    { rx:/\b(5plus|five\s+years?|5\s+plus\s+years?)/i,                                                val:"Mid-level" },
    { rx:/\b6[\s-]+(years?|yrs?)/i,                                                                    val:"Mid-level" },
    { rx:/\b4plus\s*(years?|yrs?)/i,                                                                   val:"Mid-level" },
    { rx:/\b6plus\s*(years?|yrs?)/i,                                                                   val:"Mid-level" },
    { rx:/\b7plus\s*(years?|yrs?)/i,                                                                   val:"Senior" },
    { rx:/\b7[\s-]+(years?|yrs?)\b/i,                                                                  val:"Senior" },
    { rx:/\b8plus\s*(years?|yrs?)/i,                                                                   val:"Senior" },
    { rx:/\b9plus\s*(years?|yrs?)/i,                                                                   val:"Senior" },
    { rx:/\b(1[1-9]plus|2\dplus)\s*(years?|yrs?)?/i,                                                  val:"Senior" },
    { rx:/\b([89][\s-]*(years?|yrs?))\b/i,                                                             val:"Senior" },
    { rx:/\b(gig.?work(er)?|freelance(r)?|contract(?:or)?\b|independent.?contractor|self.?employed)\b/i, val:"Gig-worker" },
    { rx:/\b([2-9]\d[\s-]*(years?|yrs?)(\s+(of\s+)?(exp|experience))?)\b/i, val:"Senior" },
    { rx:/\b(1[0-9][\s-]*(years?|yrs?)(\s+(of\s+)?(exp|experience))?|10plus\s*(years?|yrs?)?|10\+\s*(years?|yrs?)?|veteran|decade|ten\s+years?)\b/i, val:"Senior" },
    // Director / VP / C-level / Head of → senior enough to be 10+ years
    { rx:/\b(ceo|cfo|cto|coo|cmo|ciso|cpo|cro|cdo|director|vp\b|vice\s+president|c[\s-]?level|c[\s-]?suite|head\s+of\s+\w+|chief\s+(?:executive|operating|marketing|technology|financial|data|product|revenue|people|information\s+security))\b/i, val:"Senior" },
    // Bootcamp / short courses → entry level
    { rx:/\b(bootcamp|boot\s*camp|coding\s+bootcamp|certificate\s+program|self[\s-]?taught|graduate\s+program|graduate\s+scheme|grad\s+program)\b/i, val:"Fresher" },
    { rx:/^graduate\b/i,                                                                              val:"Fresher" },
    // Months of experience → entry level (1-11 months) or 1-2 years (12-23 months)
    { rx:/\b([1-9]|1[01])\s*months?\s+(?:of\s+)?(?:\w+\s+)*(?:experience|exp)\b/i,                  val:"Fresher" },
    { rx:/\b(1[2-9]|2[0-3])\s*months?\s+(?:of\s+)?(?:\w+\s+)*(?:experience|exp)\b/i,               val:"Junior" },
    // Range patterns placed LAST so they win over individual-number matches
    { rx:/\b[01][\s-]*2\s*(years?|yrs?)\b/i,                                                          val:"Fresher" },   // 0-2 yrs → entry
    { rx:/\b3\s*[\s-]?\s*5\s*(years?|yrs?)\b/i,                                                       val:"Mid-level" },   // 3-5 yrs
    { rx:/\b5\s*[\s-]?\s*(10|seven|8|9)\s*(years?|yrs?)\b/i,                                          val:"Mid-level" },   // 5-10 yrs
    { rx:/\b7\s*[\s-]?\s*(10|12|15)\s*(years?|yrs?)\b/i,                                              val:"Senior" },  // 7-10 yrs
  ];
  ...
  if      (/\b(gigs?\b|1099|consulting\s+role|independent\s+contractor|side\s+hustle|platform\s+work|temporary|temp\b|seasonal|casual\s+work|zero[\s-]hour)\b/i.test(q)) result.employmentType = "Contract";
  ...
  // Work schedule
  if      (/\bfull[\s-]?time\b/i.test(q))  result.workSchedule = "Full time";
  else if (/\bpart[\s-]?time\b/i.test(q))  result.workSchedule = "Part time";
  else if (/\binternship\b|\bto\s+intern\b|\binterning\b|\bintern\s+at\b|\bintern\b/i.test(q)) result.workSchedule = "Internship";
...
export function filterJobs(jobs, filters) {
  return jobs.filter(job => {
    ...
    if (filters.employmentType) {
      const fe = filters.employmentType.toLowerCase();
      // "In office" maps to on-site / onsite / office in job data
      const checkValues = fe === "in office" ? ["on-site", "onsite", "on site", "office"] : [fe];
      if (!checkValues.some(v => empType.includes(v))) return false;
    }
    if (filters.schedule) {
      const sch = filters.schedule.toLowerCase().replace("-", " ");
      const workSch = (job.workSchedule || "").toLowerCase();
      const fullText = empType + " " + workSch;
      if (sch === "full time") {
        if (fullText.includes("part") || fullText.includes("intern")) return false;
      } else if (sch === "part time") {
        if (!fullText.includes("part")) return false;
      } else if (sch === "internship") {
        if (!fullText.includes("intern")) return false;
      }
    }
    if (filters.experience) {
      const exp = filters.experience.toLowerCase();
      if (exp === "gig-worker") {
        const ws = (job.workSchedule || "").toLowerCase();
        const et = (job.employmentType || "").toLowerCase();
        if (!lvl.includes("gig") && !ws.includes("gig") && !ws.includes("freelance") && !et.includes("contract") && !et.includes("freelance")) return false;
      } else {
        if (lvl && lvl !== exp) return false;
      }
    }
...
export function scoreJob(job, filters) {
  ...
  //  Experience boost
  if (filters.experience) {
    const match = (lvl === filters.experience.toLowerCase()
    );
    if (match) score += 5;
  }
...
export const RESPONSE_TEMPLATES = {
  ...
  spam: () =>
    " I can't help with that type of opportunity. I specialise in legitimate professional job searches. What role can I find for you?",
...
export function getHeadhunterQuestion(parsed, profile, matchCount) {
  ...
  if (!hasType && role)
    return `\n\n*Would you prefer remote, hybrid, or in office?*`;
