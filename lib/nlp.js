...
export function parseJobQuery(rawQuery) {
  ...
  const expPatterns = [
    ...
    { rx:/\b(junior|entry\b|associate\b)\b/i,                                                          val:"Junior" },
    ...
    { rx:/\b(gig.?work(er)?|freelance(r)?|contractor?|independent.?contractor|self.?employed)\b/i, val:"Gig-worker" },
  ...
  if      (/\b(contract(or)?|freelance|gigs?\b|1099|consulting\s+role|independent\s+contractor|side\s+hustle|platform\s+work|temporary|temp\b|seasonal|casual\s+work|zero[\s-]hour)\b/i.test(q)) result.employmentType = "Contract";
