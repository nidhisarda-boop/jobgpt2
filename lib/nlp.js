...
export function filterJobs(jobs, filters) {
  return jobs.filter(job => {
    ...
    if (filters.experience) {
      ...
      // Normalised level strings (from sidebar)
      if (exp === "entry level"  && !lvl.match(/entry|junior|intern/i))       return false;
      if (exp === "mid level"    && !lvl.match(/mid|associate|intermediate/i)) return false;
      if (exp === "senior level" && !lvl.match(/senior|lead|principal|staff/i)) return false;
      // Legacy year-based strings (fallback)
...
export function scoreJob(job, filters) {
  ...
  if (filters.experience) {
    const exp = filters.experience.toLowerCase();
    const match = (
      (exp === "entry level" && lvl.match(/entry|junior|intern/i)) ||
      (exp === "mid level"   && lvl.match(/mid|associate|intermediate/i)) ||
      (exp === "senior level"&& lvl.match(/senior|lead|principal|staff/i)) ||
