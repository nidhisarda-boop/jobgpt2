...
export const EXPLICIT_INDUSTRY_PATTERNS = [
  ...
  // Round 128: Energy & Utilities
  { cat:"Energy & Utilities", rx:/\b(energy|utilities|oil\s*(?:&|and)\s*gas|petroleum|renewable(?:\s+energy)?|solar|wind\s+(?:energy|power|turbine)|power|natural\s+gas)(\s+(jobs?|roles?|positions?|work|industry|sector|field|opportunities?))?\b/i },
  { cat:"Energy & Utilities", rx:/\b(switch|change|pivot|move|transition|go)\s+to\s+(energy|utilities|oil\s*(?:&|and)\s*gas|renewables?)\b/i },
  // Round 128: Media & Entertainment
  { cat:"Media & Entertainment", rx:/\b(entertainment|film|television|tv|gaming|music|broadcast|streaming|cinema|theater|theatre)(\s+(jobs?|roles?|positions?|work|industry|sector|field|opportunities?))?\b/i },
  { cat:"Media & Entertainment", rx:/\b(switch|change|pivot|move|transition|go)\s+to\s+(entertainment|film|media|gaming|music|broadcast)\b/i },
  // Round 128: Insurance → Financial Services
  { cat:"Financial Services",  rx:/\b(insurance|insurtech)(\s+(jobs?|roles?|positions?|work|industry|sector|field|opportunities?))?\b/i },
  // Round 128: Telecom → Technology
  { cat:"Technology",           rx:/\b(telecom(?:munications)?|telco)(\s+(jobs?|roles?|positions?|work|industry|sector|field|opportunities?))?\b/i },
];
...
export function parseJobQuery(rawQuery) {
  ...
  // Round 128: Normalize "office+remote" / "office & remote" → hybrid BEFORE punctuation strip
  q = q.replace(/\boffice\s*[+&\/]\s*remote\b/gi, "hybrid")
       .replace(/\bremote\s*[+&\/]\s*office\b/gi, "hybrid")
       .replace(/\bon[\s-]?site\s*[+&\/]\s*remote\b/gi, "hybrid")
       .replace(/\bremote\s*[+&\/]\s*on[\s-]?site\b/gi, "hybrid");
  ...
  // Round 128: If location was extracted as "Remote" and no employment type was set,
  // it's not a place — convert to employmentType. But if employmentType is already set
  // (e.g. "Part-time ... in Remote"), keep location=Remote as a secondary signal.
  if (result.location && /^remote$/i.test(result.location.trim()) && !result.employmentType) {
    result.employmentType = "Remote";
  ...
  // Round 128: Generic role noun rescue — if keyword is still null but the query
  // contains a generic role noun that was stopworded, rescue it as the keyword.
  // E.g. "engineer jobs" → kw:null (engineer stopworded) → rescue → kw:"engineer"
  // E.g. "senior manager operations" → rescue → kw:"operations manager"
  if (!result.keyword && !result.categoryExplicit) {
    const qLow = q.toLowerCase().replace(/[^a-z\s]/g, " ");
    // Generic role nouns — these are valid job searches but were stopworded to avoid
    // polluting compound queries like "software engineer in Austin"
    const RESCUE_ROLES = {
      engineer:     "engineer",
      manager:      "manager",
      management:   "manager",    // "management jobs" → manager
      analyst:      "analyst",
      specialist:   "specialist",
      coordinator:  "coordinator",
      consultant:   "consultant",
      // Note: "director" intentionally excluded — too C-level for bare standalone search
      architect:    "architect",
      administrator:"administrator",
    };
    // Department/domain qualifiers that combine with role nouns
    const DEPT_QUALIFIERS = [
      "operations","marketing","sales","finance","product","engineering",
      "project","program","data","business","supply chain","human resources",
      "hr","it","systems","network","quality","clinical","research",
      "accounting","compliance","risk","security","cloud","design",
      "content","digital","brand","growth","customer","technical",
      "general","office","facilities","construction","logistics","warehouse",
    ];
    // Scan for role nouns in the query
    for (const [word, role] of Object.entries(RESCUE_ROLES)) {
      const rx = new RegExp("\\b" + word + "\\b", "i");
      if (rx.test(qLow)) {
        // Look for a department qualifier adjacent to this role
        let bestCombo = null;
        for (const dept of DEPT_QUALIFIERS) {
          const comboRx1 = new RegExp("\\b" + dept + "\\s+" + word + "\\b", "i");
          const comboRx2 = new RegExp("\\b" + word + "\\s+(?:of\\s+)?" + dept + "\\b", "i");
          if (comboRx1.test(qLow)) { bestCombo = dept + " " + role; break; }
          if (comboRx2.test(qLow)) { bestCombo = dept + " " + role; break; }
        }
        // Check if alias map has the combo
        if (bestCombo) {
          result.keyword = KEYWORD_ALIASES[bestCombo] || bestCombo;
        } else {
          result.keyword = role;
        }
        break;
